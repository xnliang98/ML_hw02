...
data_path = './dataset/raw_data/unlabeledTrainData.tsv'
...
def main():
    ...
    # data_df = pd.read_csv(data_path, header=0, delimiter='\t')
    # data_df = pd.DataFrame({'sentence': data_df.review.values})
    # data_df['label'] = 0
    # data_df = process(data_df)
    data_str = []
    with open(data_path, 'r') as f:
        for i in f.readlines():
            data_str.append(clean_text(str(i.split("\t")[1])))
    
    with open('GloVe/raw_data.txt', 'w') as f:
        for sen in train_df.sentence.values:
            f.writelines(sen + '\n')
        for sen in test_df.sentence.values:
            f.writelines(sen + '\n')
        for sen in data_str:
            f.writelines(sen + '\n')
